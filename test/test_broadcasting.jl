@testset "Field broadcasting                    " begin
    ...
    a = ProjectedField(g, randn(ComplexF64, 3*Ny, 2, (Nz >> 1) + 1, Nt))
    ...
    @test a .+ 2 == ProjectedField(g, 3*ones(ComplexF64, 2, (Nz >> 1) + 1, Nt), modes(a))
    @test a .* 2 == ProjectedField(g, 2*ones(ComplexF64, 2, (Nz >> 1) + 1, Nt), modes(a))
    ...
    @test fun(w, u, v) == 0
    u = ProjectedField(g, randn(ComplexF64, 3*Ny, 2, (Nz >> 1) + 1, Nt))
    v = ProjectedField(g, randn(ComplexF64, 3*Ny, 2, (Nz >> 1) + 1, Nt))
    w = ProjectedField(g, randn(ComplexF64, 3*Ny, 2, (Nz >> 1) + 1, Nt))
    @test fun(w, u, v) == 0
end
