@testset "Scalar Field                          " begin
        ...
        # test scaling
        x = RPCFField(grid)
        x .= rand(ComplexF64, size(x))
        A = RPCF.FarazmandScaling(ω, β)
        y = mul!(copy(x), A, x)

        passed = true
        for ny in 1:Ny, nz in 2:((Nz >> 1) + 1), nt in 1:Nt
                if !(y[ny, nz, nt] ≈ x[ny, nz, nt]/(1 + ((nz - 1)*β)^2 + ((nt - 1)*ω)^2))
                        passed = false
                        break
                end
        end
        for ny in 1:Ny, nt in 2:((Nt >> 1) + 1)
                if !(y[ny, 1, nt] ≈ x[ny, 1, nt]/(1 + ((nt - 1)*ω)^2))
                        passed = false
                        break
                end
        end
        for ny in 1:Ny
                if !(y[ny, 1, 1] ≈ x[ny, 1, 1])
                        passed = false
                        break
                end
        end
