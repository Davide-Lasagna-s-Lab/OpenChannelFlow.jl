@testset "Spectral channel field                " begin
    ...
    @test_nowarn FTField(g, T)

    # test interfaces
    @test eltype(FTField(g, T)) == Complex{T}
    @test size(FTField(g)) == (Ny, (Nx >> 1) + 1, Nz, Nt)
    @test similar(FTField(g, A)) isa FTField{typeof(g), Float64}
    @test similar(FTField(g, A), Complex{T}) isa FTField{typeof(g), T}
    @test copy(FTField(g, A)) == FTField(g, A)
    @test zero(FTField(g, A)) == FTField(g, zero(A))
    @test abs(FTField(g, A)) == FTField(g, ComplexF64.(abs.(A)))
